...
for ($i = 0; $i < AUTHORS_COUNT; $i++) {
    ...
    $author_api_token = str_rand();
    $author_password = password_hash('change_this', PASSWORD_DEFAULT);
    $pdo->exec(<<<SQL
        INSERT INTO authors(id, name, slug, avatar, email, api_token, password, created_at, deleted_at, updated_at) 
        VALUES('$author_id', '$author_name', '$author_slug', '$author_avatar','$author_email','$author_api_token','$author_password', CURRENT_TIMESTAMP, NULL, CURRENT_TIMESTAMP)
    SQL
    );
}
